	...
	namespace FlashMemory
	{
		...
		void Flash_Chip::Execute_simulator_event(MQSimEngine::Sim_Event* ev)
		{
			...
			switch (eventType) {
                ...
                void Flash_Chip::Set_metadata(flash_die_ID_type die_id, flash_plane_ID_type plane_id, flash_block_ID_type block_id, flash_page_ID_type page_id, LPA_type lpa)
                {
                        Page* page = &(Dies[die_id]->Planes[plane_id]->Blocks[block_id]->Pages[page_id]);
                        page->Metadata.LPA = lpa;
